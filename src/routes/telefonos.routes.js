...
router.get('/:id', [
  ...
  else
  {
    ...
    return res.status(200).json({message: "Telefono encontrado", data: resultado.rows[0]});
  } catch (error) {
    return res.status(400).json({message: error.message});
  }
  }
...
router.post('/', TelefonoSchema, async(req, res) => {
  ...
  else
  {
    ...
    try {
      ...
      RETURNING *
    `, [id_persona, numero]);
    return res.status(200).json({message: "Teléfono creado correctamente", data: resultado.rows[0]});
  } catch (error) {
    return res.status(400).json({message: error.message});
  }
  }
...
router.put('/:id', [
  ...
  else
  {
    ...
    return res.status(200).json({message: "Teléfono actualizado correctamente", data: resultado.rows[0]});
  } catch (error) {
    return res.status(400).json({message: error.message});
  }
  }
